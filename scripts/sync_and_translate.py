...
TRANSLATE_KEYS={"dateLabel","tagline","section","title","dek","summary","body","context","why","watchNext","timeLabel","lastUpdatedLabel","nextUpdateLabel","subtitle","description","label","note","statusLabel"}
KEEP_KEYS={"id","desk","slug","sourceName","sourceUrl","url","image","imageAlt","audio","date","editionNumber","status","leadId","editorialStandardVersion","contentVersion","createdAt","updatedAt"}
DESK_NAMES={"world":"世界","asia":"アジア","hong-kong":"香港","japan":"日本","market-economy":"経済・世界市場","stocks":"株式ニュース","ai-tech":"AI・テクノロジー","science-new-tech":"科学・新技術","cybersecurity":"サイバーセキュリティ","software-apps":"ソフトウェア・アプリ・消費者向け技術","manga-anime":"漫画・アニメ","manchester-united":"マンチェスター・ユナイテッド","football":"サッカー","breaking-news":"速報","worth-following":"きょうの注目","upcoming-events":"今後の予定"}
...
def translate_text(text):
    ...
    for part in chunks(text):
        ...
        if not last:
            raise RuntimeError(f"Japanese translation failed: {part[:100]!r}")
        out.append(last)
        time.sleep(0.08)
    value="".join(out)
    # Do not silently publish an untranslated long sentence.
    ...
    if isinstance(obj,dict):
        ...
        for k,v in obj.items():
            if k in KEEP_KEYS:
                ...
                out[k]=translate_text(v) if isinstance(v,str) else convert(v,k)
            else:
                out[k]=convert(v,k)
        if isinstance(out.get("slug"),str) and out["slug"] in DESK_NAMES:
            out["title"]=DESK_NAMES[out["slug"]]
        return out
...
def attach_audio(data):
    if not isinstance(data,dict):return data
    for a in data.get("articles",[]):
        if a.get("id"):a["audio"]=f"audio/{a['id']}.mp3"
    return data
...
def main():
    ...
    for name in FILES:
        ...
        if name in ("latest.json","desk-latest.json","stocks-latest.json"):
            translated=attach_audio(translated)

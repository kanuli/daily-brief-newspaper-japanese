...
def fail(msg):
    raise SystemExit('SITE_QA_FAIL: '+msg)
...
def validate_html():
    system_js=(ROOT/'assets/js/system-ja.js')
    system_css=(ROOT/'assets/css/system-ja.css')
    newspaper_css=(ROOT/'assets/css/newspaper.css')
    ...
    for rel in HTML_PAGES:
        ...
        for ref in parser.refs:
            ...
            target=ROOT/clean
            if not target.exists():fail(f'{rel}: broken local reference {ref}')
...
def validate_news_and_audio():
    ...
    manifest=load_json('audio/manifest.json')
    expected=set();checked=0
    ...
    missing=expected-manifest_keys
    stale=manifest_keys-expected
    if missing:fail(f'manifest missing keys: {sorted(missing)}')
    if stale:fail(f'manifest has stale keys: {sorted(stale)}')
